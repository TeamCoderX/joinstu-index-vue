        ...
        <div class="card mb-0">
            <div class="card-body p-sm-5">
                ...
                <form id="form_proposeAdd" action="">
                    ...
                    <textarea class="form-control" id="inp_details" name="details" placeholder="簡介" required v-model="details"></textarea>
                    <div class="mb-3"></div>
                    <div class="mb-3"><textarea class="form-control" id="inp_purpose" name="purpose" rows="6" placeholder="目的" required v-model="purpose"></textarea></div>
                    <p>發起的議案需與您校內事務有關，並透過此平臺讓更多同學看見，促成進一步的討論與共識，並透過「附議」來讓校方更能聽到學生們的立場。<br><span style="color: rgb(220, 53, 69);">警語：發起後將可以被所有本校的學生看到。請勿在議案中進行人身攻擊、散佈謠言或明暗示歧視、色情等令人感到不適之題材，也不可違反相關法律，違者下架議案，情節嚴重者以封鎖帳號處分。</span></p>
                    <div>
                        <div v-if="status.personAccess==true">
                            <button v-if="status.sendingFourmData == false" class="btn btn-primary d-block w-100" type="button" @click="sendAddData()">發起議案</button>  
